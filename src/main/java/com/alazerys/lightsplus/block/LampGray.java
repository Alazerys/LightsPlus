...
public class LampGray extends Block {
	
	private final boolean isOn;

	public LampGray(boolean isOn) {
	...
	public void registerBlockIcons(IIconRegister iconRegister) {
		this.blockIcon = iconRegister.registerIcon(Reference.MOD_ID + ":" + (this.isOn ? "lamp_gray_on" : "lamp_gray_off"));
	}
	...
	public void onBlockAdded(World world, int x, int y, int z) {
		...
		if (!world.isRemote) {
			if (this.isOn && !world.isBlockIndirectlyGettingPowered(x, y, z)) {
				...
				world.setBlock(x, y, z, LPBlocks.lamp_gray_on, 0, 2);
			}
		}
	}
	...
	public void updateTick(World world, int x, int y, int z, Random rand) {
		...
		if (!world.isRemote && this.isOn && !world.isBlockIndirectlyGettingPowered(x, y, z)) {
			world.setBlock(x, y, z, LPBlocks.lamp_gray_off, 0, 2);
		}
	}
	
	public Item getItemDropped(int i, Random rand, int j) {
		return Item.getItemFromBlock(LPBlocks.lamp_gray_off);
		
	}
	
	protected ItemStack createStackedBlock(int i) {
		return new ItemStack(LPBlocks.lamp_gray_off);
	}
